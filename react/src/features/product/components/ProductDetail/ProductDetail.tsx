...
export const ProductDetail: FC<ProductDetailProps> = (props) => {
	...
	const handleMovePurchasePage = useCallback(
		() => navigate(`${NAVIGATION_PATH.PURCHASE}/${product.id}`),
		[navigate],
	);

	// ゲストユーザーはログインページへリダイレクト
	const handleMoveLoginPage = useCallback(() => navigate(`${NAVIGATION_PATH.SIGNIN}`), [navigate]);
		...
		<div className={styles.parentContainer}>
			...
			<div className={styles.childRightContainer}>
				...
				<div className={styles.iconGroup}>
					<FontAwesomeIcon icon={faStarRegular} size="2x" />
					<FontAwesomeIcon icon={faComment} size="2x" />
				</div>
				{!isAuth && (
					<CommonButton type="submit" onClick={handleMoveLoginPage}>
						購入する
					</CommonButton>
				)}
				{isAuth && (
					<CommonButton type="submit" onClick={handleMovePurchasePage}>
						購入する
					</CommonButton>
				)}
